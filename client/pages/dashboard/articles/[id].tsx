...
export default function Article(): JSX.Element {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: () => push('/dashboard/articles'),
    onError: () => {
      alert('Not saved')
    },
  })
  ...
  return (
    <div className='bg-white h-full w-full flex flex-col items-center  mx-auto p-6  overflow-hidden'>
      <Form form={form} layout='vertical' onFinish={(values: ArticleFormType) => mutate({ ...values, article_tax_enabled: taxeable })} className='w-full'>
        ...
        <Form.Item<ArticleFormType> label='Article Price' name='article_price' className='flex-1' rules={rules.price}>
        ...
        <Form.Item<ArticleFormType> label='Article Type' name='article_type' className='flex-1'>
          <Select
        ...
        <Form.Item>
          <Space>
            ...
            <Button type='default' onClick={() => push('/dashboard/articles')}>
              Cancel
            </Button>
          </Space>
        </Form.Item>
